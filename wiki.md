...
The gate is now genuinely bidirectional, matching Dot.Brain's framing — but honestly, the block-approval path currently surfaces as a bare `500` (an uncaught `RuntimeException`, same pre-existing gap the "approve an already-approved pack" case already had before this change; not fixed here, see roadmap).
...
- [x] ~~Build the outbound half of the compliance/MNPI gate~~ — built (§5): `OutboundMnpiGate` blocks approval on a content-materiality match; new gap found while building it — the block surfaces as a bare 500, not a clean 4xx
- [ ] Map `KnowledgePackApprovalService`'s `RuntimeException`s (not-pending, and now outbound-gate-rejected) to a proper 4xx JSON response in `KnowledgePackController` — found while building the outbound gate (§5), not fixed as part of it
